...
export const getListProvidersMock = () => ({next: faker.helpers.arrayElement([faker.random.word(), null]), providers: Array.from({ length: faker.datatype.number({ min: 1, max: 10 }) }, (_, i) => i + 1).map(() => ({id: faker.random.word(), type: faker.random.word()}))})
