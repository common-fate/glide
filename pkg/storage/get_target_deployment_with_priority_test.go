...
func TestGetHighestPriorityDeployment(t *testing.T) {
	...
	LowPriority := targetgroup.TestTargetGroupDeployment("t1", func(d *targetgroup.Deployment) {
		d.TargetGroupAssignment = &targetgroup.TargetGroupAssignment{TargetGroupID: tg.ID, Priority: 100, Valid: true}
		d.Healthy = true
	})
	HighPriority := targetgroup.TestTargetGroupDeployment("t2", func(d *targetgroup.Deployment) {
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			q := GetTargetGroupDeploymentWithHighestPriority{TargetGroupId: tc.targetGroup.ID}
