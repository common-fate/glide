...
var (
	ErrProviderSetupNotFound = errors.New("provider deployment not found")
)

// Create a new provider deployment.
// Checks that the provider type matches one in our registry.
func (s *Service) Create(ctx context.Context, providerType string, existingProviders deploy.ProviderMap, r providerregistry.ProviderRegistry) (*providerdeployment.ProviderDeployment, error) {
	...
	ps := providerdeployment.ProviderDeployment{
		ID:               newID,
		ProviderType:     providerType,
		ProviderVersion:  version,
		Status:           types.ProviderDeploymentStatusINITIALCONFIGURATIONINPROGRESS,
		ConfigValues:     map[string]string{},
		ConfigValidation: map[string]providerdeployment.Validation{},
	}
	...
	if confvalider, ok := reg.Provider.(providers.ConfigValidator); ok {
		...
		for k, v := range validations {
			ps.ConfigValidation[k] = providerdeployment.Validation{
				Name:            v.Name,
	...
	for _, s := range steps {
		...
		ps.Steps = append(ps.Steps, providerdeployment.StepOverview{
			Complete: false,
		})
	}
