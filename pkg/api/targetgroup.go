...
func (a *API) AdminGetTargetGroupArgs(w http.ResponseWriter, r *http.Request, tgId string) {
	ctx := r.Context()
...
func (a *API) AdminListTargetGroupArgOptions(w http.ResponseWriter, r *http.Request, tgId string, argId string, params types.AdminListTargetGroupArgOptionsParams) {
	ctx := r.Context()
	// prov, ok := config.Providers[providerId]
	// if !ok {
	// 	apio.Error(ctx, w, apio.NewRequestError(&providers.ProviderNotFoundError{Provider: providerId}, http.StatusNotFound))
