...
type ProviderDeployment struct {
	...
	Status           types.ProviderDeploymentStatus `json:"status" dynamodbav:"status"`
...
func (s *ProviderDeployment) DDBKeys() (ddb.Keys, error) {
	keys := ddb.Keys{
...
func (s *ProviderDeployment) ToAPI() types.ProviderDeployment {
	ret := types.ProviderDeployment{
		Id:      s.ID,
		Type:    s.ProviderType,
...
func (s *ProviderDeployment) UpdateValidationStatus() {
	for _, v := range s.ConfigValidation {
		if v.Status == ahtypes.ERROR {
			s.Status = types.ProviderDeploymentStatusVALIDATIONFAILED
			return
		}
	...
	s.Status = types.ProviderDeploymentStatusVALIDATIONSUCEEDED
}

// func (s *ProviderDeployment) ToProvider() deploy.Provider {
