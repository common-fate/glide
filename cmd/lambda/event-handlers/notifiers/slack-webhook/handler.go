...
import (
	...
	slacknotifier "github.com/common-fate/granted-approvals/pkg/notifiers/slack-webhook"
...
func (h *handler) handleEvent(ctx context.Context, event events.CloudWatchEvent) error {
	notifier := &slacknotifier.SlackWebhookNotifier{
		DB:          h.DB,
	...
	slackCfg, ok := notificationsConfig[slacknotifier.NotificationsTypeSlackWebhook]
	if !ok {
