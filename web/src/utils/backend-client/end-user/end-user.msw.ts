...
export const getListUserAccessRulesMock = () => ({
  accessRules: Array.from(
    ...
    target: {
      ...
      with: {
        cl9dsgsfz0001kpone0mo6n9b: {
          values: Array.from(
            { length: faker.datatype.number({ min: 1, max: 10 }) },
            (_, i) => i + 1
          ).map(() => faker.random.word()),
          groupings: {
            cl9dsgsfz0000kpon2ids05pi: Array.from(
              { length: faker.datatype.number({ min: 1, max: 10 }) },
              (_, i) => i + 1
            ).map(() => faker.random.word()),
          },
