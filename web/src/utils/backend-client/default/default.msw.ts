...
import { RequestStatus, ApprovalMethod, AccessRuleStatus } from ".././types";
...
export const getAdminArchiveAccessRuleMock = () => ({
  ...
  target: {
    ...
    with: {
      cl9dsgsij000fkponaxxj9m7k: {
        values: Array.from(
          { length: faker.datatype.number({ min: 1, max: 10 }) },
          (_, i) => i + 1
        ).map(() => faker.random.word()),
        groupings: {
          cl9dsgsij000ekpon7oaj41dq: Array.from(
            { length: faker.datatype.number({ min: 1, max: 10 }) },
            (_, i) => i + 1
          ).map(() => faker.random.word()),
        },
...
export const getCreateProvidersetupMock = () => ({
  id: faker.random.word(),
...
export const getAccessRuleLookupMock = () =>
  Array.from(
    { length: faker.datatype.number({ min: 1, max: 10 }) },
    (_, i) => i + 1
  ).map(() => ({
    accessRule: {
      ...
      target: {
        provider: { id: faker.random.word(), type: faker.random.word() },
        with: {
          cl9dsgsju000qkponczo5dmny: {
            values: Array.from(
              { length: faker.datatype.number({ min: 1, max: 10 }) },
              (_, i) => i + 1
            ).map(() => faker.random.word()),
            groupings: {
              cl9dsgsju000pkponajsrfq1w: Array.from(
                { length: faker.datatype.number({ min: 1, max: 10 }) },
                (_, i) => i + 1
              ).map(() => faker.random.word()),
            },
